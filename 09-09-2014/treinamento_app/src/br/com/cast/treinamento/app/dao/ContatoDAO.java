...
//			Cursor cursorRawQuery = db.rawQuery(sqlTodos, null);
	...
	public List<Contato> pesquisarContatos(String filtroNome, String filtroTelefone) throws SQLException {
		SQLiteDatabase db = super.getReadableDatabase();
		try {			
			// Opação 2: método db.rawQuery (Inplicitamente utilizado pelo db.query):
			String sqlTodos = String.format("SELECT * FROM %s WHERE nome = '%s' AND telefone = '%s'", ContatoEntity.TABELA, filtroNome, filtroTelefone);
			Cursor cursorRawQuery = db.rawQuery(sqlTodos, null);
